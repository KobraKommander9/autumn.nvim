...
function M.get(spec, config)
	...
	return {
		...
		diffAdded = { fg = git.add }, -- Added lines ("^+.*" | "^>.*")
		diffRemoved = { fg = git.removed }, -- Removed lines ("^-.*" | "^<.*")
		diffChanged = { fg = git.changed }, -- Changed lines ("^! .*")
		diffOldFile = { fg = diag.warn }, -- Old file that is being diff against
		diffNewFile = { fg = diag.hint }, -- New file that is being compared to the old file
		diffFile = { fg = diag.info }, -- The filename of the diff ("diff --git a/readme.md b/readme.md")
