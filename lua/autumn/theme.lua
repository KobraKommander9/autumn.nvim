...
function autumn.build_syntax_groups(theme, p)
  theme.Comment = { fg = colors.light_gray4, gui = "italic" }

	theme.Constant = { fg = colors.light_magenta }
	theme.String = { fg = colors.light_green }
	theme.Character = { fg = colors.light_green }
	theme.Number = { link = "Constant" }
	theme.Boolean = { fg = colors.light_purple }
	theme.Float = { link = "Number" }

	theme.Identifier = { fg = colors.light_secondary1 }
	theme.Function = { fg = colors.light_secondary2 }

	theme.Statement = { fg = colors.light_primary2, gui = "bold" }
	theme.Operator = { fg = colors.light_primary2 }
	theme.Keyword = { link = "Statement" }

	theme.PreProc = { fg = colors.primary }
	theme.Define = { link = "PreProc" }
	theme.Macro = { link = "PreProc" }

	theme.Type = { fg = colors.light_primary1 }

	theme.Special = { fg = colors.light_blue }
	theme.SpecialChar = { link = "Special" }
	theme.Delimiter = { fg = colors.white }

	theme.Error = { fg = colors.red.readable(), bg = colors.red }
	theme.Todo = { fg = colors.light_gray2, gui = "bold" }
end

function autumn.build_diagnostic_groups(theme, p)
	theme.DiagnosticError = { fg = colors.light_red }
	theme.DiagnosticWarn = { fg = colors.light_yellow }
	theme.DiagnosticInfo = { fg = colors.light_blue }
	theme.DiagnosticHint = { fg = colors.light_purple }
	theme.DiagnosticOk = { fg = colors.light_green }
	theme.DiagnosticUnderlineError = { sp = colors.light_red, gui = "underline" }
	theme.DiagnosticUnderlineWarn = { sp = colors.light_yellow, gui = "underline" }
	theme.DiagnosticUnderlineInfo = { sp = colors.light_blue, gui = "underline" }
	theme.DiagnosticUnderlineHint = { sp = colors.light_purple, gui = "underline" }
	theme.DiagnosticUnderlineOk = { sp = colors.light_green, gui = "underline" }
end

function autumn.build_treesitter_groups(theme, p)
		theme["@variable"] = { fg = colors.white }
		theme["@variable.builtin"] = { fg = colors.light_pink }
		theme["@variable.parameter"] = { fg = colors.orange }
		theme["@variable.parameter.builtin"] = { link = "@variable.parameter" }
    theme["@variable.member"] = { fg = colors.light_orange }
    
		theme["@constant"] = { link = "Constant" }
		theme["@constant.builtin"] = { link = "Boolean" }
    theme["@constant.macro"] = { link = "Boolean" }
    
		theme["@module"] = { fg = colors.white }
		theme["@module.builtin"] = { fg = colors.white_orange }
    theme["@label"] = { fg = colors.white }
    ...
    theme["@type.definition"] = { fg = colors.green }
    ...
    theme["@punctuation.special"] = { link = "@punctuation.delimiter" }
		...
		theme["@comment.error"] = theme:extend("Comment", { sp = colors.light_red, gui = "italic,underline" })
		theme["@comment.warning"] = theme:extend("Comment", { sp = colors.light_yellow, gui = "italic,underline" })
		theme["@comment.todo"] = theme:extend("Comment", { sp = colors.light_blue, gui = "bold,italic,underline" })
