...
local function M.hsl_to_rgb(hsl)
  local r, g, b
...
local function decorate_hsl(hsl)
  ...
  return setmetatable({}, {
    __index = function(_, key)
      if key == "h" then return hsl.h end
      if key == "s" then return hsl.s end
      if key == "l" then return hsl.l end
      if key == "hsl" then return { h = hsl.h, s = hsl.s, l = hsl.l } end
      if key == "hex" then return M.hsl_to_hex(hsl) end
      if key == "rgb" then return M.hsl_to_rgb(hsl) end
...
local function hsl(hue_or_hex, saturation, light)
  ...
  if type(hex_str) == "string" then
    hsl = utils.convert.hex_to_hsl(hex_str)
  else
    if type(hue) ~= "number" or
      type(saturation) ~= "number" or
      type(light) ~= "number" then
      error("hsl() expects either a hex string or three numbers", 2)
    end

    hsl = { h = hue, s = saturation, l = light }
  end

  return decorate_hsl(hsl)
end
